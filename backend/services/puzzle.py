...
from backend.api.schemas.puzzles import Puzzle, PuzzleTestPD, UserSolution,SubmissionToAdd


class PuzzleService:
    model = Puzzles

    def __init__(self, puzzle_repo = None,
                 puzzle_test_repo = None,
                 submission_repo = None) -> None:
    ...
    async def send_user_solution(self, puzzle_id: int,
        ...
        new_submission_db = await self.submission_repo.add_one(new_submission.model_dump())

        await asyncio.sleep(0.1)  # 100 миллисекунд
