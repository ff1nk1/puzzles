...
from backend.core.custom_exceptions import UserAlreadyExistsError, UserNotFoundError, WrongPasswordError, \
    RefreshTokenNotFoundError, RefreshTokenExpiredError
...
class UserService:
    def __init__(self, user_repo: UserRepository,jwt_repo: JWTRepository|None= None):
    ...
    async def add_user(self, user_to_add: UserRegister):
        ...
        user = dict()
        if await self.check_username_and_email(user_dict['username'], user_dict['email']):
    ...
    async def login(self,user_data):
        ...
        if not await SecurityService.verify_password(user_data.password, user.hashed_password):
            raise WrongPasswordError()

        jwt_service = JWTService()
        access_token = await jwt_service.create_access_token(data={"sub": user.username})
        refresh_token= await jwt_service.create_refresh_token(data={"sub": user.username})
        ...
        refresh_expires_at = datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(
            minutes=JWTService.REFRESH_TOKEN_EXP_MINUTES)
        print("SAVED HASH:", hashed_refresh_token)
        schema_obj = RefreshTokenToAdd(user_id=user.id,
    ...
    async def refresh(self,refresh_token:str):
        try:
            ...
            new_access_token = await jwt_service.create_access_token(data={"sub": username})
            new_refresh_token = await jwt_service.create_refresh_token(data={"sub": username})

            await self.jwt_repo.make_revoke_true(refresh_token_db)
            await self.jwt_repo.delete_one(refresh_token_db)

            new_hashed_refresh_token = await jwt_service.hash_refresh_token(new_refresh_token)
            schema_obj = RefreshTokenToAdd(user_id=user_db.id,
                                           token_hash=new_hashed_refresh_token,
                                           expires_at=datetime.now() + timedelta(minutes=JWTService.REFRESH_TOKEN_EXP_MINUTES))
            await self.jwt_repo.add_one(data=schema_obj.model_dump())
            return {
