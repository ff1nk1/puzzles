...
import hashlib
from backend.core.custom_exceptions import RefreshTokenNotFoundError, RefreshTokenExpiredError
...
class JWTService:
    ...
    async def create_access_token(self,data: Dict):
        to_encode = data.copy()
        expire_time = datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(minutes=self.ACCESS_TOKEN_EXPIRE_MINUTES)
    ...
    async def create_refresh_token(self,data: Dict):
        to_encode = data.copy()
        expire_time = datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(minutes=self.REFRESH_TOKEN_EXP_MINUTES)
