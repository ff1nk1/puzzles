...
class UserRepository(SQLAlchemyRepository):
    ...
    async def get_one_by_username(self,username:str) -> Users | None:
        user = await self.session.execute(select(self.model).where(self.model.username == username))
        user = user.scalar_one_or_none()
        return user

    async def get_one_by_email(self, email: str) -> str | None:
        email = await self.session.execute(select(self.model.email).where(self.model.email == email))
